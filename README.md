...
**功能特性：**
- 🔍 **自动检测**: 自动识别上传的PDF是文本PDF还是扫描PDF
- 📝 **文本PDF**: 直接提取文本内容，立即进行RAG向量化处理
- 🖼️ **扫描PDF**: 自动使用OCR引擎提取文字，然后进行RAG处理
- ⚡ **高效处理**: 文本PDF跳过OCR步骤，大幅提升处理速度
- 📊 **详细反馈**: 返回处理状态、字符数量、置信度等详细信息

**检测标准：**
- 提取文本长度 > 50个字符
- 字母数字字符比例 > 60% 或中文字符比例 > 20%
- 自动过滤低质量文本（避免乱码文档）
...
```json
{
  ...
  "file_type": ".pdf",
  "is_pdf": true,
  "is_text_pdf": true,  // true: 文本PDF, false: 扫描PDF, null: 未检测
  "text_content": "提取的文本内容...",
  "char_count": 1500,
  "processing_status": "文本PDF - 直接RAG处理",
