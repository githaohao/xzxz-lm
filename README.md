...
本系统实现了完整的前端缓存与后端聊天历史的双向同步机制：

#### 同步流程
1. **初始化加载**：应用启动时先从本地缓存加载对话
2. **后端同步**：自动从聊天历史服务获取最新会话列表
3. **智能合并**：比较本地与远程数据，执行增量更新
4. **缓存更新**：同步完成后立即保存到本地缓存
...
// 手动强制同步
const result = await conversationStore.forceSync()
if (result.success) {
  console.log('同步成功')
} else {
...
// 检查同步状态
const isEnabled = chatStore.isHistorySyncEnabled
```
...
```typescript
// 启用/禁用历史同步
chatStore.toggleHistorySync(false) // 禁用同步
chatStore.toggleHistorySync(true)  // 启用同步
```